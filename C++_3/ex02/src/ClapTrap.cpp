...
/*   Updated: 2023/11/29 14:41:18 by raphael          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ClapTrap::takeDamage(unsigned int amount) {
	...
	std::cout << CYAN << name << " takes " << amount << " points of damage!" << NC << std::endl;
	hitPoints -= amount;
	if (hitPoints <= 0) {
		if (hitPoints > 0)
			hitPoints = 0;
		std::cout << RED << name << " is dead..." << NC << std::endl;
	}
}

void	ClapTrap::beRepaired(unsigned int amount) {
	if (energyPoints > 0) {
		energyPoints -= 1;std::cout << PURPLE << name << " heals by " << amount << NC << std::endl;
		hitPoints += amount;
		energyPoints -= 1;
		return ;
	}
