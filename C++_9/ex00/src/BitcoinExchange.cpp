...
/*   Updated: 2024/03/18 19:11:47 by rciaze           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	BitcoinExchange::interpretInput(const char *path) {
	...
	if (!getline(inputFile, line) || line.compare("date | value") != 0)
		throw std::runtime_error("Input file problem");
	while (getline(inputFile, line)) {
		int matched = sscanf(line.c_str(), "%4d%c%2d%c%2d%c%c%c%f%s", &year, &firstSeparator, &month, &secondSeparator, &day, &firstSpace, &pipe, &secondSpace, &value, str);
		if (matched == 9  && firstSeparator == '-' && secondSeparator == '-' && firstSpace == ' ' && pipe == '|' && secondSpace == ' ')
		{
			...
			if (year < 2009 || (year == 2009 && month == 1 && day < 2) || month > 12 || day > 31 || (year % 4 && month == 2 && day > 28)
